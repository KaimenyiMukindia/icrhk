	...
	$.extend(
		EnviraBox.prototype,
		{
			...
			init: function () {
				...
				var testWidth, $container, buttonStr, page = false;

				var urlParams = new URLSearchParams(window.location.search);

				if ( urlParams !== undefined && urlParams.has('envira_page') ) {
					page = urlParams.get('envira_page');
				}
			...
			getFitPos: function (slide) {
				...
				if ( isChrome && matchMedia(query).matches ) {
					imgWidth = imgWidth * 2;
			...
			setVideo: function (slide) {
				...
				if (slide.opts.insideCap === true) {
					let caption = slide.caption !== undefined ? slide.caption : '',
						...
						output = capTitleShow == 'title_caption' ? '<div class="envirabox-caption ' + 'envirabox-caption-item-id-' + itemID + '">' + caption + '</div>' : output;
			...
			updateControls: function (force) {
				...
				var html_decoded_caption = htmlDecode(caption);
