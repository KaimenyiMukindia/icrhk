...
class Shortcode {
	...
	public function shortcode( $atts ) {
		...
		if ( $markup && $should_cache && ( ! defined( 'ENVIRA_DEBUG' ) || ! ENVIRA_DEBUG ) ) {
			...
			$gallery_config   = "data-gallery-config='" . envira_get_gallery_config( $options_id, false, $data, $this->is_mobile ) . "'";
	...
	public function generate_gallery_item_markup( $gallery, $data, $item, $id, $i, $images = false ) {
		...
		$image_src_retina = apply_filters( 'envira_gallery_output_item_src_retina', envira_get_image_src( $id, $item, $data, false, true ), $item, $id, $data, $i );
		...
		if (
			...
			if (
				...
				||
				( $this->is_mobile && isset( $this->gallery_data['config']['gallery_automatic_title_caption_mobile'] ) && envira_get_config( 'gallery_automatic_title_caption_mobile', $data ) === 'title' && isset( $item['caption'] ) && isset( $item['title'] ) )
		...
		$lightbox_caption = isset( $data['config']['lightbox_title_caption'] ) && 'title_caption' === $data['config']['lightbox_title_caption'] ? $item['title'] . apply_filters( 'envira_gallery_output_lightbox_title_caption_separator', ': ', $lightbox_caption, $data ) . $lightbox_caption : $lightbox_caption;
		...
		if ( intval( envira_get_config( 'columns', $data ) ) === 0 ) {
			...
			$srcset = ( false === $srcset && ! empty( $image_src_retina ) ) ? esc_url( $image_src_retina ) . ' 2x' : $srcset;
