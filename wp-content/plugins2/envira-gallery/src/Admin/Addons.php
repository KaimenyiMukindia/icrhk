...
class Addons {
	...
	public function get_addons() {
		...
		foreach ( (array) $addons as $i => $addon ) {
			...
			if ( (
				empty( $type ) ||
				( in_array( 'advanced', $addon->categories, true ) && 'gold' !== $type && 'platinum' !== $type && 'pro' !== $type && 'ultimate' !== $type && 'agency' !== $type ) ||
				( in_array( 'basic', $addon->categories, true ) && ( 'silver' !== $type && 'gold' !== $type && 'platinum' !== $type && 'plus' !== $type && 'pro' !== $type && 'ultimate' !== $type && 'agency' !== $type ) ) ) &&
				( 'envira-social' === $addon->slug && 'plus' !== $type ) // last one, see GH #3709.
			) {
				// Unlicensed.
