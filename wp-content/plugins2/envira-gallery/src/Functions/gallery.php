...
function envira_santitize_caption( $caption ) {
	...
	$encoding = ( function_exists( 'mb_detect_encoding' ) && mb_detect_encoding( $caption ) !== 'ASCII' ) ? mb_detect_encoding( $caption ) : 'utf-8';
	...
	$caption  = envira_replace_linebreaks_with_brs( $caption );
...
function envira_santitize_title( $title ) {
	...
	$encoding       = ( function_exists( 'mb_detect_encoding' ) && mb_detect_encoding( $title ) !== 'ASCII' ) ? mb_detect_encoding( $title ) : 'utf-8';
...
function envira_get_gallery_images( $gallery_id, $raw = false, $data = null, $return_sort_ids = false, $for_albums = false, $gallery_type = false, $cache = true ) {
	...
	if ( isset( $data['gallery'] ) && is_array( $data['gallery'] ) ) {
		...
		foreach ( (array) $data['gallery'] as $id => $item ) {
			...
			if ( 'title_caption' === envira_get_config( 'lightbox_title_caption', $data ) ) {
				$title_to_combine   = isset( $item['title'] ) ? envira_santitize_caption( $item['title'] ) : false;
				$caption_to_combine = isset( $item['caption'] ) ? envira_santitize_caption( $item['caption'] ) : false;
				$caption            = implode( ' ', array( $title_to_combine, $caption_to_combine ) );
			}

			$the_data = ( false !== $album_data ) ? $album_data : $data;

			/* caption - set default then override if title was selected. */
			switch ( envira_get_config( 'lightbox_title_caption', $the_data ) ) {
				case 'caption':
				...
				case 'title_caption':
					$title_to_combine   = isset( $item['title'] ) ? envira_santitize_caption( $item['title'] ) : false;
					$caption_to_combine = isset( $item['caption'] ) ? envira_santitize_caption( $item['caption'] ) : false;
					$item['caption']    = implode( ' ', array( $title_to_combine, $caption_to_combine ) );
					break;
				default:
					$item['caption'] = false;
					break;
			...
			$item['opts']       = array(
				'caption' => ( ! empty( $item['caption'] ) ) ? envira_santitize_caption( $item['caption'] ) : '',
				'thumb'   => $thumb,
				'title'   => ( ! empty( $item['title'] ) ) ? envira_santitize_title( $item['title'] ) : '',
			);
