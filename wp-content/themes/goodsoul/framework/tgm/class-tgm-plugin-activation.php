...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
  ...
  class TGM_Plugin_Activation {
    ...
    public function load_textdomain() {
      ...
      if (
          ( is_string( $file ) && is_string( WP_PLUGIN_DIR ) && false !== strpos( $file, WP_PLUGIN_DIR ) ) ||
          ( is_string( $file ) && is_string( WPMU_PLUGIN_DIR ) && false !== strpos( $file, WPMU_PLUGIN_DIR ) )
      ) {
          // Plugin, we'll need to adjust the file name.
          add_action( 'load_textdomain_mofile', array( $this, 'correct_plugin_mofile' ), 10, 2 );
          load_theme_textdomain( 'tgmpa', dirname( __FILE__ ) . '/languages' );
    ...
    public function overload_textdomain_mofile( $mofile, $domain ) {
      ...
      // Current fallback file is not valid, let's try the alternative option.
      if ( is_string( $mofile ) && false !== strpos( $mofile, '/themes/' ) ) {
          return str_replace( '/themes/', '/plugins/', $mofile );
      } elseif ( is_string( $mofile ) && false !== strpos( $mofile, '/plugins/' ) ) {
          return str_replace( '/plugins/', '/themes/', $mofile );
      } else {
          return $mofile;
      }
    ...
    static $url;
        ...
        if ( ! isset( $url ) ) {
            ...
            if ( ! is_string( $parent ) || false === strpos( $parent, '.php' ) ) {
                $parent = 'admin.php';
            }
