 ...
 * Adds  widget.
 */
class Wpsm_Tabs_pro_Widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'wpsm_tabs_pro_widget_id', // Base ID
            'Tabs Pro Widget', // Name
            array( 'description' => __( 'Display Your Tabs in widget area.', wpshopmart_tabs_pro_text_domain ), ) // Args
        );
	}
    ...
    public function widget( $args, $instance ) {
        $Title    	=   apply_filters( 'wpsm_tabs_pro_widget_title', $instance['Title'] );
		echo $args['before_widget'];
		
		 $wpsm_tabs_r_id	=   apply_filters( 'wpsm_tabs_pro_widget_shortcode', $instance['Shortcode'] ); 

		if(is_numeric($wpsm_tabs_r_id)) {
			if ( ! empty( $instance['Title'] ) ) {
			echo $args['before_title'] . apply_filters( 'widget_title', $instance['Title'] ). $args['after_title'];
			}
			echo do_shortcode( '[TABS_PRO id='.$wpsm_tabs_r_id.']' );
		} else {
    ...
    public function form( $instance ) {
        ...
        <p>
			 ...
			 */
			$wpsm_ac_cpt = "tabs_pro";
			global $All_Wpsm_Acsh;
			$no_of_posts = wp_count_posts( 'tabs_pro')->publish;
			...
			<select id="<?php echo $this->get_field_id( 'Shortcode' ); ?>" name="<?php echo $this->get_field_name( 'Shortcode' ); ?>" style="width: 100%;">
				...
				?>
				<option value="<?php echo $PostId; ?>" <?php if($Shortcode == $PostId) echo 'selected="selected"'; ?>><?php if($PostTitle) echo $PostTitle; else _e("No Title", wpshopmart_tabs_pro_text_domain); ?></option>
				<?php endwhile; ?>
				<?php
				}  else  { 
...
// Register Widget
function Wpsm_Tabs_pro_Widget() {
    register_widget( 'Wpsm_Tabs_pro_Widget' );
}
add_action( 'widgets_init', 'Wpsm_Tabs_pro_Widget' );
?>