	...
	<div class="wpsm_sb_section_overlay_<?php echo $PostId;?>">
		...
		<div class='<?php echo $sb_wrapper_cls;?>' id='<?php echo $sb_wrapper_id;?>'> 
			...
			{  	
				...
				//for image
				if($sb_all_contents_btn_img_or_icon!='Icon'){$var_img_service_icon_cls= "img-wpsm_service_icon";}
				
				?>
				<div class="<?php echo $sb_Layout_Cls;?>">
					<div class="wpsm_serviceBox_<?php echo $PostId;?>" id="wpsm_serviceBox_<?php echo $PostId."_".$i;?>">
						<div class="wpsm_service_icon sb_icon_<?php echo $PostId."_".$i;?> serviceBox_same_height<?php echo $PostId;?>" >
							<div class="front-content <?php echo $var_img_service_icon_cls;?>">		
						...
						<div class="wpsm_service_content sb_content_<?php echo $PostId."_".$i;?> serviceBox_same_height<?php echo $PostId;?>">	
							<h3 class="wpsm_title"><?php echo $sb_all_contents_title;?></h3>
...
{ 	?>
	...
	var col = new jColumn();
	col.jcolumn('serviceBox_same_height<?php echo $PostId;?>');
		wpsm_div_heights();
	//});
<?php 
} 
...
function wpsm_div_heights(){
	var sb_TotalCount=<?php echo $sb_TotalCount;?>;
	for(i=1;i<=sb_TotalCount;i++)
	{	
		var sb_icon='.sb_icon_<?php echo $PostId;?>_'+i;
		var sb_content='.sb_content_<?php echo $PostId;?>_'+i;
		var h1= jQuery(sb_icon).height();
		var h2= jQuery(sb_content).height();
		var top_mrgin_i;
		var top_mrgin_img;
		if(h2>h1)
			h1=h2;
		
		jQuery(sb_icon).height(h1);
		jQuery(sb_content).height(h1);
		top_mrgin_i=(h1/2);
		jQuery(sb_icon+' .front-content').css({"top":""+top_mrgin_i+"px"});
	}
}
</script>	