										...
										<div class="cer-kamgc-hero-badges">
											...
											<?php if ( $cer_emblem_markup ) : ?>
												<div class="cer-kamgc-emblem-badge">
													...
													<div><strong><?php echo esc_html( $hero_badge_text ); ?></strong>
													<!--<span>Official Conference Emblem</span><small>KAMGC Kenya 2026</small>-->
													</div>
												</div>
											<?php else : ?>
												<div class="cer-kamgc-badge"><span class="material-symbols-outlined">campaign</span><span>National Summit</span></div>
											<?php endif; ?>
											<!--<div class="cer-kamgc-convening-badge; display:none;"><span class="material-symbols-outlined">workspace_premium</span><span>National Convening • Kenya 2026</span></div>-->
										</div>
										<!--<div class="cer-kamgc-activism-badge; display: none;"><span class="material-symbols-outlined">campaign</span><span>16 Days of Activism Flag-off</span></div>-->
										<h1>
							...
							<?php if ( $show_objectives && ! empty( $event_objectives ) ) : ?>
								<section class="cer-kamgc-objectives" id="objectives">
									<div class="cer-kamgc-card cer-kamgc-objectives-card">
										<span class="cer-kamgc-blobs" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></span>
										...
										<?php if ( $show_event_information ) : ?>
											<div class="cer-kamgc-card">
												...
												<div class="cer-kamgc-stats-grid">
													<div><strong>2 Days</strong><span>Plenary keynotes, panels &amp; workshops</span></div>
									...
									<div class="cer-kamgc-sidebar-column">
										<?php if ( ! empty( $event_sponsors ) ) : ?>
											<div class="cer-kamgc-sponsor-box">
												<h3 style="font-size: 25px; font-weight: bold;"><?php echo esc_html( $sponsorship_heading ); ?></h3>
							...
							<?php if ( $show_partners && ! empty( $event_partners ) ) : ?>
								<section class="cer-kamgc-partners" id="partners">
									<div class="cer-kamgc-card cer-kamgc-partners-card">
										...
										<div class="cer-kamgc-partner-grid" aria-label="<?php esc_attr_e( 'Event partners', 'custom-event-registration' ); ?>" tabindex="0">
											<?php foreach ( $event_partners as $partner ) : ?>
												<?php
												$partner_url = isset( $partner->url ) ? trim( (string) $partner->url ) : ( isset( $partner->link_url ) ? trim( (string) $partner->link_url ) : '' );
												$partner_email = isset( $partner->email ) ? trim( (string) $partner->email ) : '';
												$partner_tel = isset( $partner->tel_no ) ? trim( (string) $partner->tel_no ) : '';
												$partner_address = isset( $partner->address ) ? trim( (string) $partner->address ) : '';
												?>
												<article class="cer-kamgc-partner-card">
													<div class="cer-kamgc-partner-logo">
														<?php if ( $partner->logo_id ) : ?>
															<?php echo wp_get_attachment_image( $partner->logo_id, 'medium', false, array( 'loading' => 'lazy' ) ); ?>
														<?php else : ?>
													...
													<?php if ( $partner_address ) : ?><p class="cer-kamgc-partner-detail"><?php echo esc_html( $partner_address ); ?></p><?php endif; ?>
												</article>
											<?php endforeach; ?>
										</div>
									</div>
